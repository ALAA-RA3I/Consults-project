...
class Expert_Auth extends Controller
{
    ...
    public function Expert_register(Request $request){
      $valid=Validator::make($request->all(),['name'=>'required|string',
  ...
  public function Complet_expert_info(Request $request){
    $valid=Validator::make($request->all(),['consulting_type'=>'required|string',
    'bio'=>'required|string',
...
$expert=Expert::where('id',$ex->id)->update(['consulting_type'=>$request->input('consulting_type'),
